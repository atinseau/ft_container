...
namespace ft
{
	...
	struct tree
	{
		...
		explicit tree(const Compare & comp = Compare()): _root(NULL), _comp(comp) {}
		tree(const tree & other) : _root(_rclone(other._root)), _comp(other._comp) {}
		...
		private:
			...
			{
				if (_comp(_entry->data.first , next->data.first))
				{
					if (_entry->right)
					{
